...
def predict_probs(model, dataloader, out_dir, device, optimizer=None):
	...
	for i, seqs in enumerate(tqdm(dataloader, ncols=100, unit="batch")): #assumes motif_mask is bool tensor
		...
		seqs = F.pad(seqs, (0, 1), value=1) #add 1 to the end for the sep token
		...
		for pos in range(seq_len):
			...
			with torch.no_grad():
				logits = model(masked_seqs, None).logits
				probs = F.softmax(logits, dim=-1).to(dtype=torch.float32)[:,:-1,7:11].permute(0,2,1)
		...
		one_hot = one_hot[:,:-1,:].cpu().numpy(force=True)
