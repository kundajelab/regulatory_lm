...
def run_epoch(model, dloader, device, mask_prob, repeat_weight, num_real_tokens, optimizer=None):
    '''
    ...
    loss_fn = torch.nn.CrossEntropyLoss(ignore_index=num_real_tokens, reduction="none")
    batch_losses = []
    for i, (seqs, is_repeat) in enumerate(tqdm(dloader, ncols=100, unit="batch")):
        ...
        loss_unreduced = loss_fn(logits.permute(0,2,1), seqs_labels)
        repeat_weight_mask = torch.where(is_repeat==1, repeat_weight, 1) #Weights repeats differently than the rest
        loss = torch.mean(repeat_weight_mask * loss_unreduced)
...
def train_model(model, train_loader, valid_loader, num_epochs, output_dir, early_stopping, patience, float_dtype, mask_prob, repeat_weight, num_real_tokens, optimizer_cls, optimizer_params):
    ...
    with open(log_path, "w") as f:
        ...
        for epoch in range(num_epochs):
            ...
            model.train()
            train_batch_losses = run_epoch(model, train_loader, device, mask_prob, repeat_weight, num_real_tokens, optimizer)
            train_epoch_loss = np.nanmean(train_batch_losses)
            # print_and_log("Train: epoch %d: average loss = %6.10f" % (epoch, train_epoch_loss), txt_writer)

            with torch.no_grad():
                model.eval()
                valid_batch_losses = run_epoch(model, valid_loader, device, mask_prob, repeat_weight, num_real_tokens, optimizer=None)
            valid_epoch_loss = np.nanmean(valid_batch_losses)
            # print_and_log("Valid: epoch %d: average loss = %6.10f" % (epoch, valid_epoch_loss), txt_writer)

            print(f"Epoch {epoch}: train_loss = {train_epoch_loss}, valid_loss = {valid_epoch_loss}")
            f.write(f"{epoch}\t{train_epoch_loss}\t{valid_epoch_loss}\n")
            f.flush()
...
def main():
    ...
    train_model(model, train_loader, valid_loader, args["num_epochs"], run_dir, args["early_stopping"], args["patience"], float_dtype, args["mask_prob"], args["repeat_weight"], args["num_real_tokens"], optimizer, args["optimizer_params"])
